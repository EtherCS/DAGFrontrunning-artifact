...
class Committee:
    ...
    def __init__(self, addresses, base_port, attack_types):
        ...
        assert all(isinstance(x, list) and len(x) > 1 for x in addresses.values())
        assert all(isinstance(x, str) for y in addresses.values() for x in y)
        assert len({len(x) for x in addresses.values()}) == 1
        assert isinstance(base_port, int) and base_port > 1024
        
        port = base_port
        self.json = {"authorities": OrderedDict()}
        for (name, hosts), attack_type in zip(addresses.items(), attack_types):
            host = hosts.pop(0)
            primary_addr = {
