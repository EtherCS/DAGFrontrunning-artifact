...
class Bench:
    ...
    def _background_run(self, host, command, log_file):
        ...
        c = Connection(host, user="ubuntu", connect_kwargs=self.connect, connect_timeout=MY_CONNECTION_TIMEOUT)
    ...
    def _config(self, hosts, node_parameters, bench_parameters):
        ...
        node_num = bench_parameters.nodes[0]
        attack_types = []
        if attack_type == 0:
        ...
        committee = Committee(addresses, self.settings.base_port, attack_types)
    ...
    # to save money, we only download the attacker's log, as we dont care about the performance
    def _logs(self, committee, faults, attack_type, arbitragers):
        # Delete local logs (if any).
        cmd = CommandMaker.clean_logs()
        subprocess.run([cmd], shell=True, stderr=subprocess.DEVNULL)

        # convert attacking type for log download
        log_attack_type = 0
        if attack_type == 1:
            log_attack_type = 1
        elif attack_type == 2:
            ...
            print("It should be a valid attacking type: 1, 2, 3, or 10 \n")
            
        # Download log files.
        workers_addresses = committee.attacker_workers_addresses(log_attack_type, faults)
        progress = progress_bar(workers_addresses, prefix="Downloading workers logs:")
        attacker_index = committee.size() - faults - arbitragers
        for _, addresses in enumerate(progress):
            for id, address in addresses:
                host = Committee.ip(address)
                c = Connection(host, user="ubuntu", connect_kwargs=self.connect)
                c.get(
                    PathMaker.client_log_file(attacker_index, id),
                    local=PathMaker.client_log_file(attacker_index, id),
                )
                c.get(
                    PathMaker.worker_log_file(attacker_index, id),
                    local=PathMaker.worker_log_file(attacker_index, id),
                )

        primary_addresses = committee.attacker_primary_addresses(log_attack_type, faults)
        progress = progress_bar(primary_addresses, prefix="Downloading primaries logs:")
        for _, address in enumerate(progress):
            host = Committee.ip(address)
            c = Connection(host, user="ubuntu", connect_kwargs=self.connect)
            c.get(PathMaker.primary_log_file(attacker_index), local=PathMaker.primary_log_file(attacker_index))
    ...
    def run(self, bench_parameters_dict, node_parameters_dict, debug=False):
        ...
        for n in bench_parameters.nodes:
            ...
            for r in bench_parameters.rate:
                Print.heading(
                    f"\nRunning {n} nodes, attack_type: {attack_type}, arbitragers: {arbitragers}, workers: {workers}, faults {faults}, (input rate: {r:,} tx/s)"
                )
                ...
                for i in range(bench_parameters.runs):
                    ...
                    try:
                        ...
                        faults = bench_parameters.faults
                        logger = self._logs(
                            committee_copy, faults, attack_type, arbitragers
                        )
                        logger.print(
