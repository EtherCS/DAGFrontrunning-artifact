...
impl Proposer {
    ...
    pub async fn run(&mut self) {
        ...
        self.victim_header.author = self.name;
        ...
        loop {
                ...
                || self.attacker.attack_type() == SPECULATIVE_NOT_DELEGATE_ATTACK
            {
                ...
                if (timer_expired || enough_digests) && enough_parents && self.is_attacking {
                    self.payload_size = self.make_minimum_header().await;

                    // Reschedule the timer.
                    let deadline = Instant::now() + Duration::from_millis(self.max_header_delay);
                    timer.as_mut().reset(deadline);
                } else if (timer_expired || enough_digests) && enough_parents {
                    // Make a new header.
                    self.make_header().await;
                    self.payload_size = 0;
            ...
            tokio::select! {
                Some((parents, round)) = self.rx_core.recv() => {
                    ...
                    // TODO: make sluggish attack work
                    ...
                    // Note that for an honest validator
                    // if self.round < round, the validator will skip the round (self.round+1)
                    // In this case, he will move to the latest round (round+1)
